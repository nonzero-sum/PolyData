    ...
    def filter_search(self, queryset, _name, value):
        ...
        return (
            queryset.filter(
                ...
                | Q(source=ParadeDB(Match(normalized_value, operator="AND")))
                | Q(type=ParadeDB(Match(normalized_value, operator="AND")))
                | Q(coverage=ParadeDB(Match(normalized_value, operator="AND")))
                | Q(author=ParadeDB(Match(normalized_value, operator="AND")))
                | Q(editor=ParadeDB(Match(normalized_value, operator="AND")))
                | Q(other_colabs=ParadeDB(Match(normalized_value, operator="AND")))
                | Q(language=ParadeDB(Match(normalized_value, operator="AND")))
            )
